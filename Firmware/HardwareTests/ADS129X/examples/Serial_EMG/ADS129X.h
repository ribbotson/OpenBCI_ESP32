...
class ADS129X {
    public:
        ...
        void begin(void);
        void hw_reset(void);
        void csLow(uint8_t ads);
        void csHigh(uint8_t ads);
        
        // ADS129X SPI Command Definitions (Datasheet, Pg. 35)
        // System Commands
        void WAKEUP(uint8_t ads);
        void STANDBY(uint8_t ads);
        void RESET(uint8_t ads);
        void START(uint8_t ads);
        void STOP(uint8_t ads);

        // Data Read Commands
        void RDATAC(uint8_t ads);
        void SDATAC(uint8_t ads);
        void RDATA(uint8_t ads);

        // Register Read/Write Commands
        uint8_t RREG(uint8_t ads, uint8_t _address);
        void RREG(uint8_t ads, uint8_t _address, uint8_t _numRegisters, uint8_t *_data); //to read multiple consecutive registers (Datasheet, pg. 38)
        void WREG(uint8_t ads, uint8_t _address, uint8_t _value);

        // Functions for setup and data retrieval
        uint8_t getDeviceId(uint8_t ads);
        bool getData(uint8_t ads, int32_t *buffer);
        void configChannel(uint8_t ads, uint8_t _channel, bool _powerDown, uint8_t _gain, uint8_t _mux);
